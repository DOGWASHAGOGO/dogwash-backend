...
function formatAddons(addons) {
  ...
  const names = { cond: 'Conditioner', ear: 'Ear cleaner', perf: 'Perfume', shed: 'De-shedding treatment', lick: 'Licky mat' };
...
async function sendCustomerConfirmation(booking) {
  ...
  await transport.sendMail({
    from: `Dog Wash A Go Go <${process.env.FROM_EMAIL}>`,
    to: booking.customer.email,
    subject: `Your booking is confirmed! 🛁`,
    html: `
      <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; color: #1A1A2E;">
        <div style="background: #09A6E3; padding: 28px 32px; border-radius: 16px 16px 0 0;">
          <h1 style="color: white; margin: 0; font-size: 24px; letter-spacing: 0.5px;">DOG WASH A GO GO</h1>
        ...
        <div style="background: #FEFDF8; padding: 28px 32px; border-radius: 0 0 16px 16px; border: 1px solid #DDECF2;">
          ...
          <div style="background: #FFF9E6; border-radius: 12px; padding: 14px 18px; margin-bottom: 20px; font-size: 13px; color: #555;">
            <strong>Good to know:</strong> Please arrive a couple of minutes early so we can get you set up. Everything you need is provided — shampoo, towels, and dryer. Just bring your pup!
          </div>

          <p style="font-size: 13px; color: #888; margin: 0;">Need to cancel or reschedule? Just reply to this email and we'll sort it out.</p>
...
async function sendOwnerNotification(booking) {
  ...
  await transport.sendMail({
    from: `Dog Wash A Go Go Bookings <${process.env.FROM_EMAIL}>`,
